...
export function validateDoc(content: string, file: string): ValidationIssue[] {
  ...
  const fm = fmMatch[1];
  const get = (k: string) => fm.match(new RegExp(`^\\s*${k}\\s*:(.*)$`, "m"))?.[1]?.trim();

  const docType = (get("doc_type") || "").replace(/^["']|["']$/g, "") as DocType;
  if (!docType) {
...
/** 递归校验 docs/ 目录下所有 .md */
export async function validateDocs(root: string): Promise<ValidationResult> {
  const result: ValidationResult = { files: 0, issues: [] };
  ...
  async function walk(dir: string) {
    ...
    for (const name of list) {
      ...
      if (!stat) continue;
      if (stat.isDirectory()) {
        if (name === "node_modules" || name === "templates") continue;
        await walk(abs);
