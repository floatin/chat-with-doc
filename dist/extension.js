...
export default function (pi) {
    ...
    pi.registerCommand("doc", {
        description: "chat-with-doc: 在隔离 worktree 中进入一个模式工作 (discuss|design|experiment|produce|maintain) <task>",
        handler: async (args, ctx) => {
            const parts = args.trim().split(/\s+/);
            if (parts.length < 2) {
                ctx.ui.notify("用法: /doc <discuss|design|experiment|produce|maintain> <task>", "error");
                return;
            }
            ...
            ctx.ui.notify(`[chat-with-doc] 已进入 ${modeName} 模式 (agent ${result.data?.id ?? "unknown"})`, "info");
        },
    });
    // ---------- doc_new: 按模板创建文档 ----------
    pi.registerTool({
        ...
        description: "按指定 doc_type 模板创建一份符合 frontmatter schema 的 markdown 文档",
        parameters: {
            type: "object",
            properties: {
                doc_type: {
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
            const { doc_type, title, path, applies_to = [] } = params;
            const ok = await ctx.workspace.writeFile(path, renderTemplate(doc_type, {
                title: title,
    ...
    pi.registerTool({
        ...
        description: "扫描 worktree 中的 markdown 产物, 按 frontmatter 标记分类 (memory/core-file/branch/discard)",
        parameters: {
            type: "object",
            properties: {
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
            ...
            const plan = await scanAndClassify(params.worktree);
