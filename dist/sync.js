...
export function parseMarker(content) {
    ...
    if (!syncLine || syncLine[1] !== "true")
        return { sync: false, target: "discard" };
    const targetLine = fm.match(/^\s*target:\s*(memory|core-file|branch)\s*$/m);
    const target = targetLine ? targetLine[1] : "discard";
...
export function classifyArtifacts(artifacts) {
    ...
    for (const a of artifacts) {
        ...
        const item = { path: a.path, marker };
        if (marker.sync) {
            if (marker.target === "memory")
                plan.memory.push(item);
            else if (marker.target === "core-file")
                plan.coreFiles.push(item);
            else if (marker.target === "branch")
                plan.branch.push(item);
            else
...
export async function applyMemorySync(plan, repoRoot) {
 ...
 */
export async function scanAndClassify(worktree) {
    const artifacts = [];
